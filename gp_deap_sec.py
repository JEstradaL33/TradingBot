...
def pearson_corr(estimator, x, y, cv_iter):

    scores = []
    for train, test in cv_iter:
        estimator.fit(x.iloc[train, :], y.iloc[train])
        scores.append(pearsonr(estimator.predict(x.iloc[test, :]), y.iloc[test])[0])
    if any(np.isnan(np.array(scores))):
        raise
    else:
...
def _wrapped_cross_val_score(sklearn_pipeline, features, target,
                             ...
                             groups=None, use_dask=False, close_prices=None):
    ...
    if use_dask:
        ...
        try:
            with warnings.catch_warnings():
                ...
                CV_score_mean = pearson_corr(estimator=clone(sklearn_pipeline), x=features, y=target, cv_iter=cv_iter)
